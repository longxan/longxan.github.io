...
$(document).ready(function () {
    editJs = new Edit();
    _dataMap.put("_version", "logo_8.3");
    editJs.init();
});
...
function Edit() {
    ...
    this.updateMode = function (id, newValue) {
        ...
        if ($("#" + id).attr("comp_type") == "AnalogSliderComponent") {
            if (newValue == "true") {
                $("#" + id + " .analog_slider_comp span").css("cursor", "pointer");
            } else {
                $("#" + id + " .analog_slider_comp span").css("cursor", "auto");
            }
        }
    }
